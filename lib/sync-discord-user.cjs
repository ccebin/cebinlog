 ...
 */
async function syncUserDataWithBackoff(id, maxRetries = 12) {
  let attempts = 0;
  while (attempts <= maxRetries) {
    try {
      return await syncUserData(id);
    } catch (err) {
...
async function bulkSyncDiscordUsers(ids, options = {}) {
  ...
  for (let i = 0; i < uniq.length; i += conc) {
    ...
    const settled = await Promise.allSettled(chunk.map((id) => syncUserDataWithBackoff(id)));
